...
class ProfileDao extends DatabaseAccessor<AppDatabase> with _$ProfileDaoMixin {
  ...
  Future<DbProfile?> getActiveProfile() {
    return (select(dbProfiles)..where((t) => t.isActive.equals(true))).getSingleOrNull();
  }

  Future<int> createProfile({required String name, required String avatar}) async {
    await transaction(() async {
      await (update(dbProfiles)).write(const DbProfilesCompanion(isActive: Value(false)));
    });
    final id = await into(dbProfiles).insert(DbProfilesCompanion.insert(
      name: name,
      avatar: avatar,
  ...
  Future<void> resetProfileData(int profileId) async {
    await (delete(dbSessionAnswers)..where((t) => t.sessionId.isInQuery(
      selectOnly(dbSessions)
